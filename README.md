...
> **🧪 MySQL is in BETA (v2.3.0).** Core CDC is functional: binlog streaming (ROW format, GTID-aware), restart from saved `(binlog_file, position, gtid_executed)` checkpoint, **and concurrent snapshot + CDC** via the read-only DBLog Incremental Snapshot algorithm (Debezium 2022 design — per-chunk GTID watermarks with binlog-event-wins-on-PK-collision reconciliation). Requires `gtid_mode=ON` and `binlog_format=ROW`. TLS verification is on by default; set `MYSQL_TLS_SKIP_VERIFY=true` only for dev environments. Production deployments will exercise edge cases on schema/type coverage that aren't yet covered by integration tests — feedback and issues welcome via [Discussions](https://github.com/ez-cdc/dbmazz/discussions).
