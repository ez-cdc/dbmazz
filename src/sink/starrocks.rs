...
use sonic_rs::{json, Object as Map, Value};
...
use crate::sink::Sink;
use crate::source::parser::{CdcMessage, Tuple, TupleData};
...
impl StarRocksSink {
    ...
    fn tuple_to_json(&self, tuple: &Tuple, schema: &TableSchema) -> Result<Map> {
        self.tuple_to_json_selective(tuple, schema, false)
            .map(|(row, _)| row)
    }
    ...
    async fn send_body_with_retry(
        ...
        loop {
            match self
                .curl_loader
                .send(table_name, body.clone(), partial_columns.clone())
                .await
            {
                Ok(_) => return Ok(()),
                Err(e) => {
                    ...
                    info!(
                        "Retry {}/{} for {}: {}",
                        attempt, max_retries, table_name, e
                    );
    ...
    async fn send_with_retry(
        ...
        self.send_body_with_retry(table_name, body, None, max_retries)
            .await
    }
    ...
    async fn send_partial_update_with_retry(
        ...
        self.send_body_with_retry(table_name, body, Some(columns_vec), max_retries)
            .await
    }
}
                ...
                CdcMessage::Update {
                    ...
                    if let Some(schema) = schema_cache.get(*relation_id) {
                        ...
                        let (mut row, columns) = if has_toast {
                            ...
                            let (r, mut cols) =
                                self.tuple_to_json_selective(new_tuple, schema, true)?;
                        ...
                        let entry = batches
                            .entry(key)
                            .or_insert_with(|| (Vec::new(), columns.clone()));
                        entry.0.push(row);
                    }
                }
        ...
        for (key, (rows, columns)) in batches {
            if let Some(schema) = schema_cache.get(key.relation_id) {
                if let Some(cols) = columns {
                    // Partial update
                    self.send_partial_update_with_retry(&schema.name, rows, &cols, 3)
                        .await?;
                } else {
                    // Full row
